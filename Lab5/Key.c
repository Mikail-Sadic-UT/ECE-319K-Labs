...
// I.e., PortB has already been reset and activated (do not reset PortB here again)
  // write this
    //Keys
    IOMUX->SECCFG.PINCM[PB16INDEX] = 0x0040081; //Key0=311.1
    IOMUX->SECCFG.PINCM[PB17INDEX] = 0x0040081; //Key1=392.0
    IOMUX->SECCFG.PINCM[PB18INDEX] = 0x0040081; //Key2=466.2
    IOMUX->SECCFG.PINCM[PB19INDEX] = 0x0040081; //Key3=523.3 Hz
}
...
uint32_t Key_In(void){
    uint32_t keyData = (GPIOB->DIN31_0 >> 16) - 0x20;
    return keyData; // replace with solution
}
